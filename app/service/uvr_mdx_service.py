...
class UvrMdxService:
    ...
    def __init__(self, args={
            "model_path": MODEL_DICT['uvr-mdx-net-inst-hq-3'],
            "denoise": True,
            "margin": 44100,
            "chunks": 15,
            "n_fft": 6144,
    ...
    def separate(self, audio_bytes: bytes) -> bytes:
        ...
        audio_array = np.apply_along_axis(soxr.resample, axis=0, arr=audio_array,
                                          in_rate=16000, out_rate=44100,
                                          quality='soxr_hq')
        vocal_array, _ = self.predict(mix=audio_array)
        vocal_array = np.apply_along_axis(soxr.resample, axis=0, arr=vocal_array.mean(axis=1),
                                          in_rate=44100, out_rate=16000,
                                          quality='soxr_hq')
        vocal_array = (vocal_array * 32768.0).astype(np.int16)
        return vocal_array.tobytes()


uvr_mdx_service = UvrMdxService()


if __name__ == "__main__":
    with open(r'example.pcm', 'rb') as pcm:
        audio_bytes = pcm.read()
    vocals = uvr_mdx_service.separate(audio_bytes)
    with open(r'vocals.pcm', 'wb') as pcm:
        pcm.write(vocals)
